...
(function initializeNexusOneLineModule() {
  ...
  function bindEvents(instance) {
    ...
    bindAction(instance, "save", function handleSave() {
      ...
      showToast(instance, "Diagram saved locally.");
    });
  ...
  function storageLoad(instance) {
    ...
    try {
      ...
      /**
       * The existing storage adapter accepts only diagramId.
       * Fall through to legacy loading.
       */
    }

    try {
      return adapter.load(instance.diagramId);
    } catch (legacyError) {
  ...
  function storageSave(instance, layout) {
    ...
    if (!adapter || typeof adapter.save !== "function") {
      throw new Error(
        "NexusOneLineStorage is not available."
      );
    }
    ...
    try {
      adapter.save(context, layout);
      return;
    } catch (contextError) {
      /**
       * The current adapter expects save(diagramId, layout).
       */
    }

    adapter.save(instance.diagramId, layout);
  }
  ...
  function saveDiagramInstance(instance, manualSave) {
    ...
    try {
      storageSave(instance, clone(instance.state));
  ...
  function getFitItems(instance) {
    ...
    const minimumX = Math.min(
      ...items.map(function mapMinimumX(item) {
        return item.x;
      })
    );

    const minimumY = Math.min(
      ...items.map(function mapMinimumY(item) {
        return item.y;
      })
    );

    const maximumX = Math.max(
      ...items.map(function mapMaximumX(item) {
        return item.x + (item.w || 180);
      })
    );

    const maximumY = Math.max(
      ...items.map(function mapMaximumY(item) {
        return item.y + (item.h || 100);
      })
    );
    ...
    instance.transform.x =
      (
        rectangle.width -
        contentWidth * instance.transform.scale
      ) /
        2 -
      minimumX * instance.transform.scale;

    instance.transform.y =
      (
        rectangle.height -
        contentHeight * instance.transform.scale
      ) /
        2 -
      minimumY * instance.transform.scale;
